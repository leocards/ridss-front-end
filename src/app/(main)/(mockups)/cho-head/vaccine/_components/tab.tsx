...
interface Props {
    tab: 'inventory' | 'request' | 'issuance' | 'items' | 'oversight'
}
export default function VaccineTab({ tab }: Props) {
    ...
    return (
        <ArkUITabButton activeValue={active} onValueChange={({ value }) => {
            ...
            <ArkUITabItem value="oversight" className={cn(active === "oversight" ? "text-primary-foreground" : "hover:text-foreground")}>
                <span className="relative z-10">Oversight</span>
            </ArkUITabItem>
        </ArkUITabButton>
    )
}